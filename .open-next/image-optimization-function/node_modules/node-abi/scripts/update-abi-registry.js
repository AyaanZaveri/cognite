...
async function fetchNodeVersions() {
  ...
  for (const [majorVersion, metadata] of Object.entries(schedule)) {
    ...
    const lts = metadata.hasOwnProperty("lts")
      ? [metadata.lts, metadata.maintenance]
      : false;
    versions[version] = {
      runtime: "node",
      target: version,
...
async function fetchAbiVersions() {
  return (
    await getJSONFromCDN("nodejs/node/doc/abi_version_registry.json")
  ).NODE_MODULE_VERSION.filter(({ modules }) => modules > 66);
}
...
function electronReleasesToTargets(releases) {
  ...
  return Object.entries(versionsByModules).map(
    ([major, { version, modules }]) => ({
      abi: modules,
      future: !versions.find((v) => {
...
async function main() {
  ...
  const supportedTargets = [...nodeTargets, ...electronTargets];

  await writeFile(
    path.resolve(__dirname, "..", "abi_registry.json"),
    JSON.stringify(supportedTargets, null, 2)
  );
}
