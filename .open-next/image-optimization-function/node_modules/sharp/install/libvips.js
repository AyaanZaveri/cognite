...
const localLibvipsDir =
  process.env.npm_config_sharp_libvips_local_prebuilds || "";
const distHost =
  process.env.npm_config_sharp_libvips_binary_host ||
...
const distBaseUrl =
  ...
  `${distHost}/v${minimumLibvipsVersionLabelled}/`;
...
const extractTarball = function (tarPath, platformAndArch) {
  ...
  const ignoreVendorInclude =
    hasSharpPrebuild.includes(platformAndArch) &&
    ...
    function (err) {
      if (err) {
        if (/unexpected end of file/.test(err.message)) {
          fail(
            new Error(`Please delete ${tarPath} as it is not a valid tarball`)
          );
        }
        fail(err);
      }
...
try {
  ...
  if (useGlobalLibvips) {
    ...
    if (platformAndArch === "darwin-arm64") {
      throw new Error(
        "Please run 'brew install vips' to install libvips on Apple M1 (ARM64) systems"
      );
    }
    if (
    ...
    if (!semverSatisfies(process.versions.node, supportedNodeVersion)) {
      handleError(
        new Error(
          `Expected Node.js version ${supportedNodeVersion} but found ${process.versions.node}`
        )
      );
    }
    ...
    const tarFilename =
      ["libvips", minimumLibvipsVersionLabelled, platformAndArch].join("-") +
      ".tar.br";
    ...
    if (fs.existsSync(tarPathCache)) {
      ...
      simpleGet(
        { url: url, agent: agent(libvips.log) },
              ...
              .on("close", function () {
                if (!response.complete) {
                  tmpFileStream.destroy(
                    new Error("Download incomplete (connection was terminated)")
                  );
                }
              })
              .pipe(tmpFileStream);
